        ...
        http
                ...
                .authorizeHttpRequests(req ->
                        req     .antMatchers(SWAGGER_URLS).permitAll()
                                ...
                                .requestMatchers(swaggerRequestMatcher("/api/v1/management/**","/api/v1/bookings/**")).hasAnyRole(ADMIN.name(), MANAGER.name())
                                .requestMatchers(swaggerRequestMatcher(String.valueOf(GET), "/api/v1/management/**","/api/v1/bookings/**")).hasAnyAuthority(ADMIN_READ.name(), MANAGER_READ.name())
                                .requestMatchers(swaggerRequestMatcher(String.valueOf(POST), "/api/v1/management/**","/api/v1/bookings/**")).hasAnyAuthority(ADMIN_CREATE.name(), MANAGER_CREATE.name())
                                .requestMatchers(swaggerRequestMatcher(String.valueOf(PUT), "/api/v1/management/**","/api/v1/bookings/**")).hasAnyAuthority(ADMIN_UPDATE.name(), MANAGER_UPDATE.name())
                                .requestMatchers(swaggerRequestMatcher(String.valueOf(DELETE), "/api/v1/management/**","/api/v1/bookings/**")).hasAnyAuthority(ADMIN_DELETE.name(), MANAGER_DELETE.name())
                                .anyRequest()
                                .authenticated()
                )
